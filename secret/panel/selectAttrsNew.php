...
$idpage = "selectAttrsNew";
$thisTable = $selectAttrTable;
include("h.php");
        ...
        if (!is_null($category)) {
            if ($isSubCategory != '0') {
                ...
                if ($isSubCat == '0') {
                    ...
                    $ins = $db->query("INSERT INTO $thisTable (sub_cat, title,cat_id) VALUES('$subCatVal', '$title', '$category')");
        ...
        if (!is_null($category)) {
            if ($isSubCategory != '0') {
                ...
                if ($isSubCat == '0') {
                        ...
                        $edi = $db->query("UPDATE $thisTable SET sub_cat='$subCatVal',title='$title',cat_id='$category' WHERE id='$id'");
                        if ($edi) {
                            $class = 'success';
                            $content = $errorsBank[3];
...
<div class="row">
    <div class="col-lg-12">
        <form action="" method="post" role="form" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>عنوان ویژگی / زیر مجموعه</label>
                <select name="companyID" class="form-control" onchange="getModels(this,'select-icon');">
                    <option value="0" style="color: red;">برای ثبت زیر مجموعه، عنوان را انتخاب کنید</option>
                    <?php
