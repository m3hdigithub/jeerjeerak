    ...
    <div class="table-responsive">
            ...
            <form action="<?php /*echo $idpage; */?>" class="form-inline" method="post">
                <div class="form-group col-md-9 pull-right">
                    <input type="text" class="form-control" name="search"
                           placeholder="جستجو بر اساس (توضیحات، شماره پیگیری)">
                </div>
        ...
        <table class="table table-bordered table-hover table-striped tablesorter">
            ...
            <tr>
                ...
                <th>نام جیرجیرکی کاربر خریدار</th>
                <th>نام جیرجیرکی کاربر فروشنده</th>
                <th>عنوان آگهی</th>
                <th>کد شانس</th>
                <th>تعداد شانس</th>
                <th>درصد تعیین شده</th>
                <th>زمان ثبت</th>
                <th>تنطیمات</th>
            </tr>
            </thead>
            ...
            $userBuyer = getField($usersTable,'username','id',$row['user_buyer_id']);
            $userPayer = getField($usersTable,'username','id',$row['user_payer_id']);
            $adv = getField($advertisingTable,'title','id',$row['adv_id']);
            $chanceCode = checkEmptyVar($row['code']);
            $numChance = checkEmptyVar($row['num_code']);
            $darsadAdv = getField($advertisingTable,'chance','id',$row['adv_id']).'%';
            ?>

            <tr>
                <td><?php echo $i; ?></td>
                <td><a href="member?id=<?php echo $row['user_buyer_id']; ?>"><?php echo $userBuyer; ?></a></td>
                <td><a href="member?id=<?php echo $row['user_payer_id']; ?>"><?php echo $userPayer; ?></a></td>
                <td><a href="advertisingDetail?id=<?php echo $row['user_payer_id']; ?>"><?php echo $adv; ?></a></td>
                <td><?php echo $chanceCode; ?></td>
                <td><?php echo $numChance; ?></td>
                <td><?php echo $darsadAdv; ?></td>
    ...
    $('.infoBtn').click(function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                setTimeout(function () {
                    window.location.href = "order?id=" + thisId;
                }, 0);
            }
        });
    });
