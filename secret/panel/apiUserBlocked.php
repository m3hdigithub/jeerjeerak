...
if (isset($_REQUEST['changeStatus'])) {
    ...
    if ($thisID) {
        $beforStatus = $db->query("SELECT num_login FROM $userLoginBlockTable WHERE user_id='$thisID'")->fetch_all(1);
        $beforStatus = ($beforStatus) ? $beforStatus[0]['num_login'] : 'no';

        if ($beforStatus != 'no') {
            $chengingST = ($beforStatus < $apiLCAllowedNum) ? $apiLCAllowedNum : 0;
            $updated = $db->query("UPDATE $userLoginBlockTable SET num_login='$chengingST' WHERE user_id='$thisID'");
            if ($updated) {
                echo true;
            } else {
                echo false;
...
$sqll = "SELECT * FROM $userLoginBlockTable WHERE num_login >= '$apiLCAllowedNum'";

if (isset($_GET['todayUsers'])) {
    $sqll = "SELECT * FROM $userLoginBlockTable WHERE num_login >= '$apiLCAllowedNum' AND DATE(date_time) = CURDATE()";
}
$allBlockedID = $db->query($sqll)->fetch_all(1);

$allUserIDBlockeds = array();
if ($allBlockedID){
    $numBlockeds = 0;
    foreach ($allBlockedID as $bloked => $block){
        if(!isNotBlockUser($block['user_id'])){
            $allUserIDBlockeds[] = $block['user_id'];
            $numBlockeds++;
        }
    }
}
...
<div class="col-lg-12">
    <div class="alert alert-info text-center">در این قسمت لیست کاربرانی قرار می گیرد که در صدد ورود غیر مجاز به اپلیکیشن
        را داشته اند و به صورت خودکار توسط سیستم بلاک شده اند.
    </div>
    <?php if ($allBlockedID && $numBlockeds){ ?>
    <div class="btn-group">
        <a href="?todayUsers" class="btn btn-default">
            <span class=""> تعداد بلاک شده های امروز </span>
            <span class="label label-danger"><?php echo getNotifications($userLoginBlockTable); ?></span>
        </a>
    ...
    <div class="table-responsive">
            ...
            <form action="<?php echo $idpage; ?>" class="form-inline" method="post">
                <div class="form-group col-md-9 pull-right">
                    <input type="text" class="form-control" name="search"
                           placeholder="جستجو بر اساس (نام، نام خانوادگی، نام کاربری، موبایل، ایمیل، تلفن، کد ملی)">
                </div>
        ...
        <table class="table table-bordered table-hover table-striped tablesorter">
            ...
            $sql2 = "select * from $thisTable order by date_time desc limit 0,$adminPerPage";
            $sql3 = "select * from $thisTable order by date_time desc limit $start,$adminPerPage";
            $search = '1=1';
            ...
            if (isset($_POST['search']) && $_POST['search'] != '' && strlen($_POST['search']) > 0) {
                ...
                $sql2 = "select * from $thisTable WHERE $search order by date_time desc limit 0,$adminPerPage";
                $sql3 = "select * from $thisTable WHERE $search order by date_time desc limit $start,$adminPerPage";
            }
            /*if (isset($_GET['todayUsers'])) {
                $sql1 = "select * from $thisTable WHERE DATE(date_time) = CURDATE()";
                $sql2 = "select * from $thisTable WHERE DATE(date_time) = CURDATE() order by date_time desc limit 0,$adminPerPage";
                $sql3 = "select * from $thisTable WHERE DATE(date_time) = CURDATE() order by date_time desc limit $start,$adminPerPage";
            }*/
            ...
            if (in_array($row['id'], $allUserIDBlockeds)){

            $imgName = (isset($row['u_image']) && $row['u_image'] != '') ? $row['u_image'] : $defaultImage;
            $status = $row['u_status'];
            $sex = 'ثبت نشده';
            if ($row['u_sex'] == '1') {
                $sex = 'آقا';
            ...
            <tr>
                <td><?php echo $i; ?></td>
                <td><img src='<?php echo $imageShowT; ?>' width='40' height='40'
                ...
                <td>
                    <div class="btn-group">
                        ...
                        <span datatype="" class="btn btn-xs btn-info infoBtn"
                              id="<?php echo $row['id']; ?>">مشاهده اطلاعات کاربر</span>
                    </div>
